	...
	fvPatchScalarField& qheatBoundary =
        ...
        forAll(*this, i)
        {
            ...
            scalar qRadDry =
               ...
               //+temissivity[i]*sigma*pow4(operator[](i));	<- used in other turb... B.C.
	    ...
	    qheatBoundary[i] = -nbrTotalFlux[i];	    // Ning Two region Model
	    nbrTotalFlux[i] = blockingInternal[i]*temissivity[i]*sigma*(pow4(operator[](i))-pow4(surfaceTInternal[i]))
				+(1-blockingInternal[i])*nbrTotalFlux[i];
	    //nbrTotalFlux[i] = nbrTotalFlux[i]*(1.0-blockingInternal[i]);    //Ning Old Model
	    //qheatBoundary[i] = -nbrTotalFlux[i];	    // Ning Old Model
...
//            );
	...
	mpp.distribute(TsurField);	//Ning
        ...
        //this->refValue() = Twall;
        this->refValue() = TsurField;	//-Ning, use actual paper surface T
