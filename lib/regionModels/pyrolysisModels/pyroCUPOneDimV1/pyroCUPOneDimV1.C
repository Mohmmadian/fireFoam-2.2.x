...
void pyroCUPOneDimV1::initParams()
{
      ...
      if (multiFuel_)
      {
        ...
        forAll(intCoupledPatchIDs_, i)
        {
          ...
          scalarField& Cp_UC    = CpUC_.boundaryField()[patchI];
...
// 
      ...
      forAll(intCoupledPatchIDs_, i)
      {
          ...
          forAll(pPatch,faceI)
          {
            ...
            forAll(boundBoxPallets,palletID)
            {
             ...
             if (pID > -1) face2Pallet_[pID].boundaryField()[patchI][faceI] = 1.;   // setting this to 1 for the pallet this face belongs to..  
      ...
      forAll(intCoupledPatchIDs_, i)
      {
          ...
          scalarField& mOLInit_patch   = mOLInit_.boundaryField()[patchI];
          scalarField& mOLLost_patch   = mOLLost_.boundaryField()[patchI];
...
// Identifying patches on the back boundary for the outer liner; for the OL-OUC coupling 
      ...
      const volScalarField::GeometricBoundaryField& Tbf = T1.boundaryField();
...
// ------------
     ...
     forAll(intCoupledPatchIDs_, i)
     {
        ...
        scalarField& IOL_patch       = IOL_.boundaryField()[patchI];
        scalarField& IOUCHU_patch    = IOUCHU_.boundaryField()[patchI];
        scalarField& IOUCR1_patch    = IOUCR1_.boundaryField()[patchI];
        scalarField& IOUCR2_patch    = IOUCR2_.boundaryField()[patchI];
        scalarField& IIUC_patch      = IIUC_.boundaryField()[patchI];

        scalarField& pyrHOC_patch    = pyrolHOC_.boundaryField()[patchI];
...
void pyroCUPOneDimV1::updateFuelFluxMassFrac()
{
    ...
    const volScalarField::GeometricBoundaryField& fuelCC_bf = fuelCC.boundaryField();

    const volScalarField& fuelPS = gasMesh.lookupObject<volScalarField>(speciesPS_); 
    const volScalarField::GeometricBoundaryField& fuelPS_bf = fuelPS.boundaryField();
    ...
    forAll(intCoupledPatchIDs_, i)
    {
        ...
        const scalarField& ccBurntSt = swccGone_.boundaryField()[patchI];
        const scalarField& UCBurntSt = outerUCGone_.boundaryField()[patchI];
        scalarField& m_UC     = massUC_.boundaryField()[patchI];           
...
{
    qradBnd_.boundaryField()[patchI] = qrad;
}


void pyroCUPOneDimV1::setQconv(const scalarField& qconv, label patchI)
{
    qconvBnd_.boundaryField()[patchI] = qconv;
}
...
void pyroCUPOneDimV1::preEvolveRegion()
{
    ...
    forAll(intCoupledPatchIDs_, i)
    {
        ...
        scalarField& ccBurntSt = swccGone_.boundaryField()[patchI];
        scalarField& UCBurntSt = outerUCGone_.boundaryField()[patchI];
...
void pyroCUPOneDimV1::evolveRegion()
{
    ...
    scalarField& T = solidThermo_.T().internalField();
...
*/
        ...
        scalarField& T_UC     = tempUC_.boundaryField()[patchI];           
        scalarField& m_UC     = massUC_.boundaryField()[patchI];           
        const scalarField& Cp_UC    = CpUC_.boundaryField()[patchI];           
        //scalarField& alpha_UC = alphaUC_.boundaryField()[patchI];           
        //scalarField& emm_UC   = emmUC_.boundaryField()[patchI];           

        scalarField& mdot_UC  = mdotUC_.boundaryField()[patchI];           
        scalarField& mdot_IUC = mdotIUC_.boundaryField()[patchI];           
        mdot_UC  = 0.;
        mdot_IUC = 0.;
        ...
        scalarField& emmBndV   = emmBnd_.boundaryField()[patchI];           
        scalarField& absBndV   = absBnd_.boundaryField()[patchI];           
        ...
        scalarField& energyRelUC             = enerRelUC_.boundaryField()[patchI];
      ...
      else
      {
       ...
       {  
        ...
        {
          scalarField& face2Pallet_patch   = face2Pallet_[palletID].boundaryField()[patchI];
    ...
    forAll(intCoupledPatchIDs_, i)
    {
        const label patchI           = intCoupledPatchIDs_[i];
        //const scalarField& UCBurntSt = outerUCGone_.boundaryField()[patchI];
        scalarField& m_IUC           = massInnerUC_.boundaryField()[patchI];
        //const scalarField& mUC       = massUC_.boundaryField()[patchI];
        scalarField& phiGasp         = phiGas_.boundaryField()[patchI];
        scalarField& mdot_IUC        = mdotIUC_.boundaryField()[patchI];
